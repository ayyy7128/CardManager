...
- 卡片聚焦/编辑、投资详情/编辑和日历任务编辑改为 Activity 内真实全屏页面。
- 新增 Android 预测性返回手势支持，全屏页面会跟随手势进度移动并支持取消回弹。
- 底部 Tab 改为悬浮在正文上方，宽度随可见 Tab 数量自适应，并优化浅色/深色背景。
- 新增标准 `.zip` 卡片资源包导入、覆盖更新、查看占用空间和删除功能。
- 新增内置银行/发行机构目录和识别 Logo，银行选择页支持搜索、首字母分组和自定义银行。
- 新增 GitHub Releases 更新检查，设置页支持手动检查。
- 继续公开 ZIP 资源包目录与 JSON 清单规范。
